...
(function(){
  ...
  */
  ...
  /* ==========================================================
     IASYN 2 — PRESENTACIÓN CIE-10 PROFESIONAL + CATÁLOGO MAESTRO
     2026-09-09
     ----------------------------------------------------------
     - Conserva códigos compactos internamente y en persistencia.
     - Presenta al médico el CIE-10 con punto cuando corresponde:
       N760 -> N76.0, N871 -> N87.1, B977 -> B97.7.
     - Las categorías válidas de 3 caracteres permanecen sin decimal.
     - La búsqueda usa el catálogo maestro activo y no muestra categorías
       ni subcategorías técnicas en la interfaz clínica.
     - No escribe en Google Sheets ni modifica Apps Script.
  ========================================================== */
  let auroDxCatalogoMaestro = [];
  let auroDxCatalogoCargado = false;
  let auroDxPromesaCatalogo = null;
  let auroDxTokenBusqueda = 0;

  function auroDxCieCompacto(valor){
    return texto(valor).toUpperCase().replace(/[^A-Z0-9]/g,'');
  }

  function auroDxExtraerAliasVisual(registro, compacto){
    registro = registro || {};
    compacto = compacto || auroDxCieCompacto(
    ...
    const alias = hallados.find(item => auroDxCieCompacto(item) === compacto);
    return alias ? texto(alias).toUpperCase() : '';
  }

  function auroDxFormatearCie10Visual(valor, registro){
    const compacto = auroDxCieCompacto(valor);
    if(!compacto) return '';

    const aliasDirecto = auroDxExtraerAliasVisual(registro, compacto);
    if(aliasDirecto) return aliasDirecto;

    /* Si el catálogo ya está cargado, respeta primero un alias oficial. */
    try{
      const itemCatalogo = auroDxCatalogoMaestro.find(item =>
        auroDxCieCompacto(item?.codigo) === compacto
      );
      const aliasCatalogo = itemCatalogo
        ? auroDxExtraerAliasVisual(itemCatalogo.raw || itemCatalogo, compacto)
        : '';
      if(aliasCatalogo) return aliasCatalogo;
    }catch(_e){}

    /* Categorías CIE-10 de 3 caracteres permanecen sin punto. */
    if(/^[A-Z][0-9]{2}$/.test(compacto)) return compacto;

    /* Subcategorías compactas: punto tras el tercer carácter. */
  ...
  window.auroFormatearCie10Visual = auroDxFormatearCie10Visual;
  window.iasynFormatearCie10Visual = auroDxFormatearCie10Visual;

  function auroDxNormalizarRegistroCatalogo(raw){
    raw = raw || {};
    const codigo = auroDxCieCompacto(
      raw.codigo_cie10 || raw.codigo || raw.cie10 || ''
    );
    const nombre = texto(
      raw.descripcion || raw.nombre || raw.diagnostico ||
    ...
    return {
      codigo,
      nombre,
      codigo_visual:auroDxFormatearCie10Visual(codigo, raw),
      raw
    };
  }

  function auroDxCatalogoFallbackLocal(){
  ...
  function auroDxFusionarCatalogos(principal, respaldo){
    const mapa = new Map();
    [...(principal || []), ...(respaldo || [])].forEach(item => {
      const normalizado = item?.raw ? item : auroDxNormalizarRegistroCatalogo(item);
      const clave = auroDxCieCompacto(normalizado.codigo);
  ...
  async function auroDxCargarCatalogoMaestro(){
    if(auroDxCatalogoCargado && auroDxCatalogoMaestro.length){
      return auroDxCatalogoMaestro;
    }
    if(auroDxPromesaCatalogo) return auroDxPromesaCatalogo;

    auroDxPromesaCatalogo = (async () => {
      const local = auroDxCatalogoFallbackLocal();
      try{
        ...
        auroDxCatalogoMaestro = auroDxFusionarCatalogos(remotos, local);
        auroDxCatalogoCargado = auroDxCatalogoMaestro.length > 0;
      }catch(error){
        console.warn(
          MODULO + ': catálogo maestro CIE-10 no disponible; se conserva el catálogo local.',
          error
        );
        auroDxCatalogoMaestro = local;
        auroDxCatalogoCargado = auroDxCatalogoMaestro.length > 0;
      }finally{
        auroDxPromesaCatalogo = null;
      }

      /* Compatibilidad con el editor histórico: solo código + nombre. */
      if(auroDxCatalogoMaestro.length){
        window.hcCie10CatalogoBase = auroDxCatalogoMaestro.map(item => ({
          codigo:item.codigo,
          nombre:item.nombre
        }));
      }

      window.IASYN_CATALOGO_DIAGNOSTICOS_ACTIVO = auroDxCatalogoMaestro.slice();
      return auroDxCatalogoMaestro;
    })();

    return auroDxPromesaCatalogo;
  }

  function auroDxRenderResultadosBusqueda(lista){
    ...
    body.innerHTML = lista.map((item, indice) =>
      '<tr>' +
        '<td class="diagnostico-cie-code">' +
          escapeHtml(item.codigo_visual || auroDxFormatearCie10Visual(item.codigo, item.raw)) +
        '</td>' +
  ...
  async function auroDxBuscarDiagnosticoCie10IASYN(){
    const miToken = ++auroDxTokenBusqueda;
    ...
    const codigo = auroDxCieCompacto(codigoRaw);
    const nombre = normalizar(nombreRaw);

    if(!codigo && !nombre){
      window.hcDxResultadosActuales = [];
      auroDxRenderResultadosBusqueda([]);
      return [];
    }

    const catalogo = await auroDxCargarCatalogoMaestro();
    if(miToken !== auroDxTokenBusqueda) return [];

    const resultados = (catalogo || []).filter(item => {
      const coincideCodigo = !codigo || auroDxCieCompacto(item.codigo).includes(codigo);
      const coincideNombre = !nombre || normalizar(item.nombre).includes(nombre);
      return coincideCodigo && coincideNombre;
    }).slice(0,20);

    /* El guardador histórico conserva el código compacto internamente. */
    window.hcDxResultadosActuales = resultados.map(item => ({
      codigo:item.codigo,
      nombre:item.nombre
    }));

    auroDxRenderResultadosBusqueda(resultados);
    return resultados;
  }

  function auroDxRenderSeleccionadosIASYN(){
  ...
  function auroDxInstalarPresentacionCie10(){
    /*
      IASYN 2 — instalación inmediata, catálogo remoto diferido.
      ---------------------------------------------------------
      La presentación con punto queda disponible sin esperar red.
      El catálogo maestro remoto se carga al buscar un diagnóstico o después
      de pintar primero los diagnósticos de la atención. Así la lectura clínica
      inicial no compite con una consulta adicional a Apps Script.
      No modifica persistencia ni el código compacto interno.
    */
    window.buscarDiagnosticoCie10 = auroDxBuscarDiagnosticoCie10IASYN;
    window.renderDiagnosticosSeleccionados = auroDxRenderSeleccionadosIASYN;

    try{ auroDxRenderSeleccionadosIASYN(); }catch(_e){}
    return true;
  }
  /* ==========================================================
  ...
  */
  ...
  /*
    IASYN 2 — VERIFICACIÓN CLÍNICA SINCRÓNICA DEL GUARDADO
    ------------------------------------------------------
    El botón no se marca como guardado hasta que la misma id_atencion
    devuelva exactamente la lista esperada desde la fuente persistida.
  */
  function auroDxPrincipalBooleano(valor){
    if(valor === true || valor === 1) return true;
    const v = normalizar(valor);
    return ['si','sí','true','1','principal'].includes(v);
  }

  function auroDxFirmaDiagnosticos(lista){
    return (Array.isArray(lista) ? lista : [])
      .map(raw => ({
        codigo:auroDxCieCompacto(raw?.codigo_cie10 || raw?.codigo || raw?.cie10 || ''),
        nombre:normalizar(raw?.descripcion || raw?.nombre || raw?.diagnostico || ''),
        tipo:normalizar(raw?.tipo_diagnostico || raw?.tipo || 'Presuntivo') === 'definitivo'
          ? 'definitivo'
          : 'presuntivo',
        principal:auroDxPrincipalBooleano(raw?.principal)
      }))
      .filter(item => item.codigo || item.nombre)
      .sort((a,b) => {
  ...
  function auroDxEsperar(ms){
    return new Promise(resolve => window.setTimeout(resolve, ms));
  }

  async function auroDxConfirmarPersistenciaGuardadoAbierto(idAtencion, esperados){
    const id = texto(idAtencion);
    const firmaEsperada = auroDxFirmaDiagnosticos(esperados);
    const pausas = [0,180,420];
    let ultimoError = null;
    let ultimoPersistido = [];

    for(let intento=0; intento<pausas.length; intento++){
      if(pausas[intento]) await auroDxEsperar(pausas[intento]);
      try{
        const persistidos = await auroDxVerificarDiagnosticosPersistidosAbiertos(id);
        ultimoPersistido = persistidos;
        if(auroDxFirmaDiagnosticos(persistidos) === firmaEsperada){
          return {
            ok:true,
            intentos:intento + 1,
            persistidos
    ...
    try{
      ...
      const resultado = await Promise.resolve(
        window.auroGuardarDiagnosticosAtencionActual({
          ...
          /* IASYN 2: comparar contra lo REALMENTE persistido, no contra memoria. */
      ...
      */
      ...
      /*
        ...
        Solo entonces la UI adopta el estado y muestra “Diagnóstico guardado ✓”.
      */
      ...
      if(!verificacionPersistencia.ok){
        console.error(
          MODULO + ': el backend respondió, pero el diagnóstico no pudo confirmarse en la base.',
          {
            id_atencion:ctx.id,
            esperados:snapshotEditor,
            persistidos:verificacionPersistencia.persistidos,
            error:verificacionPersistencia.error || null
          }
        );
        throw new Error(
          'El sistema recibió respuesta del backend, pero no pudo confirmar en la base ' +
  ...
  */
  ...
  function diagnosticosLocales(){
    ...
    const salida = lista.map(d => ({
      ...
      principal: d.principal === true || ['si','sí','true','1'].includes(normalizar(d.principal)),
      tipo_diagnostico: texto(d.tipo_diagnostico || d.tipo || 'Presuntivo'),
      estado: texto(d.estado || 'Activo'),
      origen: 'Editor CIE-10'
    })).filter(d => d.codigo_cie10 || d.descripcion);

    /* Solo asigna el primero como principal si realmente no existe ninguno. */
    if(salida.length && !salida.some(d => d.principal)) salida[0].principal = true;
    return salida;
  }
  ...
  */
  function sincronizarEditorCie10DesdeDiagnosticos(){
    ...
    /* Reafirma el renderer profesional para que el editor siempre lea con punto. */
    try{ auroDxInstalarPresentacionCie10(); }catch(_e){}

    const origen = Array.isArray(state.diagnosticos) ? state.diagnosticos : [];
    const hayPrincipalReal = origen.some(d =>
      d?.principal === true || ['si','sí','true','1'].includes(normalizar(d?.principal))
    );

    const lista = origen.map((d, index) => ({
      ...
      principal:
        d.principal === true ||
        ['si','sí','true','1'].includes(normalizar(d.principal)) ||
        (!hayPrincipalReal && index === 0),
      tipo: normalizar(d.tipo_diagnostico || d.tipo || 'Presuntivo') === 'definitivo'
        ? 'Definitivo'
        : 'Presuntivo'
      ...
      return `
        <div class="auro-dx-item">
          <div class="auro-dx-item-main">
            <div class="auro-dx-code">${escapeHtml(auroDxFormatearCie10Visual(d.codigo_cie10, d) || 'S/C')}</div>
            ...
            <button type="button" class="auro-dx-protocol-btn" data-ver-protocolo-dx="${index}" title="Ver protocolo clínico completo" aria-label="Ver protocolo clínico completo de ${escapeHtml(auroDxFormatearCie10Visual(d.codigo_cie10, d) || d.descripcion)}">
              <i class="bi bi-eye"></i>
            </button>
          </div>
        </div>
  ...
  function renderProtocolos(){
        ...
        <div class="auro-dx-protocolo ${seleccionado ? 'selected' : ''}" data-protocolo-index="${index}">
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start">
            <div>
              ...
              <small>${escapeHtml(auroDxFormatearCie10Visual(p.codigo_cie10, p.raw || p))} · ${escapeHtml(p.especialidad)} ${p.version ? '· v' + escapeHtml(p.version) : ''}</small>
            </div>
    ...
    /*
      Mientras el médico edita una atención abierta, los protocolos pueden
      actualizarse en segundo plano, pero no deben reconstruir la cabecera ni
      reemplazar el botón de Guardar/Cancelar.
    */
    if(state.edicionDiagnosticoAbierto !== true){
      renderContextoSuperior();
    }
  ...
  */
  ...
  function construirResumenClinico(){
    ...
    if(principal){
      add('Como diagnóstico principal se registra ' +
        [auroDxFormatearCie10Visual(principal.codigo_cie10, principal), principal.descripcion].filter(Boolean).join(' - ') +
    ...
    if(secundarios.length){
      add('Se registran como diagnósticos asociados ' +
        auroListaNatural(secundarios.map(x =>
          [auroDxFormatearCie10Visual(x.codigo_cie10, x),x.descripcion].filter(Boolean).join(' - ')
        )));
    }
  ...
  function construirAnalisis(){
    ...
    */
    if(principal){
      const dx = [auroDxFormatearCie10Visual(principal.codigo_cie10, principal), principal.descripcion]
        .filter(Boolean)
        .join(' - ');
  ...
  function construirContextoApoyoIA(){
    ...
    */
    ...
    return {
      ...
      diagnostico: {
        principal: [auroDxFormatearCie10Visual(principal.codigo_cie10, principal), principal.descripcion].filter(Boolean).join(' - '),
        cie10: state.diagnosticos.map(d => auroDxFormatearCie10Visual(d.codigo_cie10, d)).filter(Boolean).join(', '),
        diferenciales: asociados.map(d => [auroDxFormatearCie10Visual(d.codigo_cie10, d),d.descripcion].filter(Boolean).join(' - ')).filter(Boolean).join('\n'),
  ...
  let auroDxUltimaLecturaDiagnosticosValida = false;
  ...
  async function consultarDiagnosticos(idAtencion){
    try{
      ...
      if(data && data.success === false){
        throw new Error(data.message || 'El backend no confirmó la lectura de diagnósticos.');
      }
      auroDxUltimaLecturaDiagnosticosValida = true;
      return normalizarDiagnosticosServidor(data);
    }catch(e){
      auroDxUltimaLecturaDiagnosticosValida = false;
      console.warn(MODULO + ': no se pudieron consultar diagnósticos persistidos.', e);
      return [];
    }
  }
  ...
  */
    ...
    const ejecutarCarga = async () => {
      try{
        ...
        */
        ...
        /*
          IASYN 2 — PRIORIDAD REAL AL DIAGNÓSTICO
          ---------------------------------------
          Primero se espera SOLO la consulta de diagnósticos. La integración
          clínica pesada se inicia después de que el CIE-10 ya fue leído.
          Así la lectura principal no compite desde el primer milisegundo con
          seis solicitudes adicionales a Apps Script.
        */
        const dxServidor = await promesaDiagnosticos;
        if(!cargaSigueVigente()) return null;

        /*
          IASYN 2 — FUENTE AUTORITATIVA POR id_atencion
          Si listarDiagnosticosPorAtencion respondió correctamente, incluso un
          resultado vacío es un resultado clínico válido y NO se mezcla con
          memoria local. Esto evita que un diagnóstico no persistido reaparezca.
          Solo si la lectura remota falla se conserva el fallback histórico.
        */
        state.diagnosticos = auroDxUltimaLecturaDiagnosticosValida
          ? clonar(dxServidor, [])
          : fusionarDiagnosticos(dxServidor, diagnosticosLocales());
        ...
        */
        ...
        /*
          El catálogo maestro CIE-10 se precarga recién DESPUÉS de mostrar el
          diagnóstico. No bloquea la atención y la búsqueda seguirá cargándolo
          por demanda si el médico empieza a escribir antes.
        */
        try{
          Promise.resolve(auroDxCargarCatalogoMaestro()).catch(() => {});
        }catch(_e){}

        /*
          FASE 2 — INTEGRACIÓN CLÍNICA EN SEGUNDO PLANO
          Examen, historia, anamnesis y especialidades empiezan ahora, cuando
          el diagnóstico principal ya está visible.
        */
        ...
        */
        if(!state.diagnosticos.length && !auroDxUltimaLecturaDiagnosticosValida){
          /*
            Fallback legado SOLO cuando la lectura autoritativa falló.
            Una respuesta remota válida con cero diagnósticos permanece en cero.
          */
        ...
        */
        ...
        /*
          IASYN 2 — PROTECCIÓN DEL EDITOR ABIERTO
          ---------------------------------------
          Si el médico empezó a editar después de que apareció el CIE-10 pero
          antes de terminar la integración, una respuesta tardía NO debe volver
          a sincronizar el editor ni reconstruir sus botones.
        */
        const editandoDiagnosticoAbierto =
          state.edicionDiagnosticoAbierto === true;

        if(!editandoDiagnosticoAbierto){
          renderDiagnosticos();
          sincronizarEditorCie10DesdeDiagnosticos();
          renderProtocolos();
        }
  ...
  function cambiarPorAtencion(idAtencion){
    ...
    const mismaAtencion = !!idAtencion && idAnterior === idAtencion;

    if(
      state.correccionClinicaActiva &&
      idAnterior &&
      idAtencion &&
      idAnterior !== idAtencion
    ){
      auroDxRestaurarPuenteGuardadoCorreccion();
      state.correccionClinicaActiva = false;
      state.correccionClinicaMeta = null;
    ...
    /*
      ...
      ------------------------------------------------
      Atenciones puede emitir varias señales mientras una consulta abierta sigue
      siendo la misma. Esas señales NO justifican limpiar ni releer Diagnóstico.

      - Si ya existe una carga de esa atención, se reutiliza.
      - Si el médico está editando, no se reconstruye su editor ni su botón.
      - Si la atención cambió de estado (p. ej. fue cerrada), se refresca solo
        la cabecera/bloqueo usando el estado maestro, sin tocar los diagnósticos.
      - Una nueva id_atencion sí ejecuta la carga clínica normal.
      - El evento específico aurosanax:diagnosticos-actualizados y el botón
        “Sincronizar datos” conservan la capacidad de forzar una lectura real.
    */
    if(mismaAtencion){
      if(state.cargando && state.idCargaEnCurso === idAtencion && state.promesaCarga){
        return state.promesaCarga;
      }

      const ctx = contextoAtencionSeleccionada();

      if(state.edicionDiagnosticoAbierto === true && ctx.editable === true){
        return state;
      }
      ...
      if(state.ultimaActualizacion || state.diagnosticos.length){
        renderContextoSuperior();
        auroDxAplicarEstadoEditorHistorico();
        configurarModoProtocoloMaestro();
        return state;
    ...
    */
    ...
    /*
      Solo una atención realmente nueva fuerza una carga completa. Para la misma
      atención sin datos aún, cargarAtencion(false) reutiliza cualquier promesa
      en curso o inicia la primera lectura si todavía no existe.
    */
    return cargarAtencion(idAtencion, !mismaAtencion);
  }
  ...
  function inicializar(){
    /* IASYN 2: instala búsqueda maestra y presentación CIE-10 con punto. */
    try{ auroDxInstalarPresentacionCie10(); }catch(error){
      console.warn(MODULO + ': no se pudo instalar la presentación CIE-10 profesional.', error);
    }
  ...
  window.auroDiagnosticos = {
    ...
    limpiarContextoHistoriaNueva,
    sincronizarEditorCie10DesdeDiagnosticos,
    formatearCie10Visual:auroDxFormatearCie10Visual,
    cargarCatalogoDiagnosticos:auroDxCargarCatalogoMaestro
  };
  ...
  */
  [0, 250, 800].forEach(ms => {
    ...
    setTimeout(auroDxInstalarPresentacionCie10, ms);
  });
